...
function initialize_parameters()
{
  ...
  log "Redefine initialize parameters from file '$1': $chef_repo_path | $project_name"
}
export -f initialize_parameters

function redefine_initialize_data()
{
  log "Redefine initialize data: $chef_repo_path | $project_name"
...
function run_new_project()
{
  echo
  log "--------------------------------------------------------------"
  log "Running new project $project_name at $chef_repo_path"
  log "--------------------------------------------------------------"
  echo
